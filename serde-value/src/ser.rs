...
impl ser::Serializer for Serializer {
    ...
    fn serialize_struct_variant(
        ...
        Ok(SerializeStructVariant(variant.to_string(), IndexMap::new()))
    }
}
...
impl ser::SerializeTupleVariant for SerializeTupleVariant {
    ...
    fn end(self) -> Result<Self::Ok, Self::Error> {
        ...
        let Value::String(key) = self.0 else {
            unreachable!();
        };

        map.insert(key, Value::Seq(self.1));
        Ok(Value::Map(map))
    }
}
...
struct SerializeStruct(IndexMap<String, Value>);
...
struct SerializeStructVariant(String, IndexMap<String, Value>);
